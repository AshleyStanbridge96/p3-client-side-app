QUnit.test( "TEST calcTotal", assert => {
  ...
  QUnit.test( "TEST calcTip", assert => {
    assert.equal(calcTotal(20, .05, 1),1, "Normal input");
    assert.equal(calcTotal(-20, .15, -10),0.3, "Negative inputs")
    assert.equal(calcTotal(30, .2, 2),3, "Normal input")
    assert.equal(calcTotal(40, .1, 4),1, "Normal input")
    assert.equal(calcTotal(50, .15, 5),1.5, "Normal input")
  });
